...
function focusReplyCardIfNeeded() {
  const card = elements.replyCard;
  if (!card) {
    return;
  }
  ...
  const isVisible = rect.top >= viewportTop + 6 && rect.bottom <= viewportBottom - 6;
  if (!isVisible) {
...
async function generateReply(reasonText, options = {}) {
  const { copyAfterGenerate = false, respectAutoCopy = true, autoTriggered = false } = options;
  if (isGenerating) {
    return;
  }
...
async function maybeAutoGenerate(previousState, nextState) {
  ...
  await generateReply("检测到新消息，已自动生成建议...", { autoTriggered: true });
}
...
async function loadInitialData() {
  ...
  if (Boolean(prefs?.autoGenerateEnabled ?? true) && hasMessage && !hasReply) {
    ...
    await generateReply("已根据最新消息自动生成建议...", { respectAutoCopy: false, autoTriggered: true });
    return;
  }
  setStatus("准备就绪");
}
